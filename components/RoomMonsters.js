...
export default RoomMonsters = ({
  ...
  const changeMob = (x, key, name) => {
    console.log("change mob");
    console.log(x);
    let tempMv = { ...mobValues };
    let temp = tempMv[key][0].split(":");
    if (name == "x") {
      delete tempMv[key];
      temp[1] = name;
      tempMv[key] = temp;
    ...
    if (name == "type") {
      let x = [temp, value];
      console.log(`${key} value ${x} name ${name}`);
      tempMv[temp] = value;
      setMobValues(tempMv);
    }
  };
        ...
        <p>
          <label>
            ...
            <input
              type="number"
              name="x"
              onChange={(e) => changeMob(e.target.value, key, e.target.x)}
          ...
          <label>
            ...
            <input
              ...
              name="type"
              onChange={(e) =>
                changeMob(e.target.value, (x = { value }), e.target.name)
              }
              value={value[1]}
            />
          </label>
