...
export const multiItems = (itemPath = [1, "items", "bag"]) => {
  ...
  let path = [];
  console.log(temp);
  let x = temp.reduce((data, key) => path.push(characterData[data]), temp);

  //let x = itemPath.reduce((data, key) => data[key], characterData);
  /*let y = temp.reduce(
    (data, key) => (path = characterData[data]),
    characterData
  );*/
  let a = 1;
  let b = "items";
...
export const multiItems = (itemPath = [1, "items", "bag"]) => {
  ...
  let path = "";
  console.log(temp);
  let x = temp.reduce((data, key) => (path = characterData[data]), temp);
